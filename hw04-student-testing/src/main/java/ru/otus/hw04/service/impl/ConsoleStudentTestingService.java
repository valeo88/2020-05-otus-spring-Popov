...
import ru.otus.hw04.config.UserTestSettings;
...
public class ConsoleStudentTestingService implements StudentTestingService {
    ...
    public void showAllQuestions() {
        try {
            ...
            ioService.writeError(getLocalizedMessage("error.loadingQuestions", e.getMessage()));
        }
    }
    ...
    private int performTesting() {
        ...
        try {
            ...
            ioService.writeText(getLocalizedMessage("error.loadingQuestions", e.getMessage()));
            return 0;
        }
        allQuestions.stream()
    ...
    private void printQuestionWithAnswers(Question question) {
        ioService.writeText(getLocalizedMessage("question.title", question.getNumber(),
    ...
    private void printResults(String userName, int correctAnswers) {
        if (correctAnswers >= userTestSettings.getMinCorrectAnswersForCredit()) {
            ioService.writeText(getLocalizedMessage("stmt.congratulation", userName));
        } else {
            ioService.writeText(getLocalizedMessage("stmt.tryAgain", userName));
        }
    }
